...
`NEXT_PUBLIC_SITE_URL` is the Next origin used by Craft preview targets. On
Vercel, the application can derive its generated domain from Vercel system
environment values when no explicit override is configured.
...
- remain dynamic and private on Vercel

Vercel Draft Mode bypasses the published ISR response when its Draft Mode
