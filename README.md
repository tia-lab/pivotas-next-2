...
```env
PROJECT_NAME=ubiq-next-cyon
```

Synchronize it with DDEV through:

```sh
bun env:create --project-name=ubiq-next-cyon
```
...
Complete this checklist before connecting Next or Vercel to hosted Craft.
...
Do not configure Next or Vercel with an HTTPS Craft endpoint while the domain
...
CRAFT_REVALIDATE_URL=https://<vercel-domain>/api/revalidate
```

Keep the generated local `NEXT_PUBLIC_SITE_URL` until the Vercel URL exists,
...
After the first Craft deployment, propagate a new Vercel callback URL without
...
After the first push, verify all three public routes before configuring Vercel:
...
Create the Vercel project from `app/` and set `app` as its root directory.
`app/vercel.json` pins the framework to `nextjs`, so Vercel must use its native
Next.js builder even when dashboard framework detection reports `Other`. Keep
the build, install, and output directory settings on their framework defaults.
Configure at least these values in the Vercel project:

```txt
CRAFT_GRAPHQL_ENDPOINT=https://<craft-domain>/gql/api
CRAFT_PRIMARY_URL=https://<craft-asset-domain>
CRAFT_GRAPHQL_TOKEN=<token-from-hosted-craft>
REVALIDATE_SECRET=<same-secret-used-by-craft>
CRAFT_PREVIEW_SECRET=<same-preview-secret-used-by-craft>
```

Do not configure `NEXT_PUBLIC_SITE_URL` for the first deployment merely to
guess Vercel's generated domain. With Vercel system environment variables
enabled, the app uses `VERCEL_PROJECT_PRODUCTION_URL`, then `VERCEL_URL`.
Configure `NEXT_PUBLIC_SITE_URL` later only when a stable custom canonical
domain should override Vercel's generated URL.

Production deployment sequence:

1. Import the GitHub repository in Vercel and select `app` as the Root
   Directory. Vercel uses the Next.js application under `app/`;
   `app/vercel.json` also pins the framework to `nextjs`.
2. Add the Vercel environment values above and deploy. When the deployment is
   live, copy its stable Production Domain and set this in the root `.env`:
   ...
   Use the stable project or custom domain, not a commit-specific Preview URL.
3. Propagate the frontend URL, callback URL, `REVALIDATE_SECRET`, and
...
Vercel owns frontend deployment and production frontend env values. The local
root `.env` is not uploaded to Vercel. The app build regenerates the ignored
GraphQL schema and TypeScript output before `next build`, so no manual Vercel
deploy or generation command is required.
...
- exports the running DDEV database, including content and GraphQL tokens
- uses the tracked clean starter database only when DDEV is not running
...
For a clean first push containing the latest local content and images, keep
DDEV running. The database contains authored content; the managed directories
contain the corresponding files. `cpresources` and generated image transforms
are not synchronized because Craft regenerates them.
...
The documented deployment uses Cyon for Craft and Vercel for Next. The
...
1. creates and syncs env files through `bun env:create`
...
removes stale generated DDEV router/env files.
...
The build command already generates CSS and runs the complete GraphQL
generation/check pipeline before `next build`.
...
needed in Vercel or Cyon.

On Vercel, the frontend site URL resolves in this order:

1. `NEXT_PUBLIC_SITE_URL` as an optional explicit canonical override
2. `VERCEL_PROJECT_PRODUCTION_URL`
3. `VERCEL_URL`

The Vercel system values do not include `https://`; the app normalizes them
before generating metadata and preview redirects.
...
configured in Next/Vercel and propagated to Craft/Cyon. Local development
derives `NEXT_PUBLIC_SITE_URL` from `DEV_PORT`; after the first Vercel
...
DDEV project name, writes `DEV_PORT=3000` and project-specific local URLs, and
generates the Craft security, revalidation, and preview secrets.
...
schema and type files stay within the Next app and the Vercel build does not
...
Generates CSS tokens, regenerates and checks GraphQL schema/types, then runs
`next build`. The configured Craft GraphQL endpoint must be reachable, return a
valid schema for the configured token, and use trusted TLS when it is HTTPS.
...
- Connect `app/` to Vercel and configure `app/` as the Vercel project root.
- Vercel owns frontend builds, domains, runtime env values, and deployments.
- The normal app build generates GraphQL artifacts before `next build`; no
  manual Vercel deploy command is required.
...
- `bun craft:push` and `bun craft:pull` own manual Craft content
  ...
  environment.
- GitHub Actions validates the repository only. It and normal frontend commits
  must not synchronize Craft.
- Review the displayed target and type the required confirmation before every
  push or pull.

For the full GitHub Actions build, configure these repository secrets:
...
If they are absent or the GraphQL probe fails, CI runs the local-safe install,
CSS, and lint checks and reports that GraphQL generation and the Next build were
skipped.
...
| `HIT`   | Vercel serves the already generated response from cache.                |
| `STALE` | Vercel serves the previous response while Next regenerates it.          |
...
`no-cache` headers are not reliable tests of the normal Vercel cache path. If
regeneration fails, Vercel keeps the last successful response and retries
...
The current model supports manual navigation items and page links. The default
main navigation handle is expected to be:
...
Because the build regenerates GraphQL artifacts, pre-push requires the Craft
GraphQL endpoint and token in `.env` to be valid and reachable. An HTTPS
endpoint must also present a trusted certificate; never bypass this with
...
`bun run build` includes CSS and GraphQL generation/checking.
...
After deploying to Vercel, test the same normal route at least twice without
Draft Mode, Vercel Toolbar cookies, or `cache-control: no-cache`. Record the
response headers and verify a regional `x-vercel-cache: HIT`. Then verify that
