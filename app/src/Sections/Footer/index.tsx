					...
					{footer.footerNavigation?.length ? (
						<div>
							{footer.footerNavigation.map((navigation) => {
								...
								return (
									<nav key={navigation.id ?? navigation.navigationHandle}>
										{navigation.title ? <h3>{navigation.title}</h3> : null}
										{navigation.navigationItems?.length ? (
											<div>
												{navigation.navigationItems.map((item) => {
													if (item?.__typename !== 'navigationItem_Entry') {
														return null
													}

													const page = item.pageLink?.[0]
													const href = item.externalUrl ?? pathFromCraftUri(page?.uri)
