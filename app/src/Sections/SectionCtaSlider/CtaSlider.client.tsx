...
export const CtaSlider = ({ children }: { children: ReactNode }) => (
	<Slider
		classes={{ controls: $.slider_controls, slide: $.slide }}
		className={$.slider}
		gap='2rem'
			...
			<>
				<div className={$.slider_dots}>
					{slider.scrollSnaps.map((_, index) => (
						<button
							aria-current={
								index === slider.selectedIndex ? 'true' : undefined
							}
							aria-label={`Go to slide ${index + 1}`}
							className={clsx(
								$.slider_dot,
								index === slider.selectedIndex && $.is_active
							)}
							key={index}
